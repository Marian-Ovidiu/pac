        ...
        <nav class="flex items-center justify-between h-16 lg:h-20">
            ...
            <div class="hidden lg:flex lg:items-center lg:ml-auto lg:space-x-10">
                @foreach($menu as $key => $item)
                    <a href="{{ $item->url }}" title="" class="text-base font-medium text-black transition-all duration-200 hover:text-blue-600 focus:text-blue-600 font-nunitoSans"> {{ $item->title }} </a>
                @endforeach
            </div>

            <a href="#" title="" class="items-center justify-center hidden px-4 py-3 ml-10 text-base font-semibold text-white transition-all duration-200 bg-blue-600 border border-transparent rounded-md lg:inline-flex hover:bg-blue-700 focus:bg-blue-700" role="button">Dona ora</a>
        </nav>
        ...
        <nav x-show="open" @click.away="open = false"
            ...
            <div class="flow-root">
                <div class="flex flex-col px-6 -my-2 space-y-1">
                    @foreach($menu as $key => $item)
                        <a href="{{ $item->url }}" title="" class="justify-center inline-flex py-2 text-base font-medium text-black transition-all duration-200 hover:text-blue-600 focus:text-blue-600"> {{ $item->title }} </a>
                    @endforeach
                </div>
            </div>
